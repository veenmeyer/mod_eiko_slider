...
;(function($){
	...
	var defaults = {
		...
		responsive: true,
        forceReloadAtResize : true, // Look function resizeWindow()
	...
	jQuery.fn.bxSlider = function(options){
		...
		var setup = function(){
            ...
            if (slider.settings.mode === 'horizontal') {
                ...
                el.children().each(function() {
                    wrapperWidth += jQuery(this).outerWidth();
                });
		...
		var appendCaptions = function(){
			...
			slider.children.each(function(index){
				...
				if (title != undefined && ('' + title).length) {
                    // Check caption
                    //var regx = /(\d{2}[.-]\d{2}[.-]\d{4}.{0,3}) - ([BGH]+[\d]?.*)/;
					//var regx = /(\d{2}[.-]\d{2}[.-]\d{4}(.*[\d]{1,2}:[\d]{1,2} Uhr)?)\s*([BGH]+[\d]?.*)/;
					var regx = /(\d{2}\.\d{2}\.\d{4}) ([BGH]+[\d]?.*)/;
					var title = title.replace(/--/g, "<br>"); 
                    var res = title.match(regx);
                    
                    if (res != null) {
                        jQuery(this).append('<div class="bx-caption"><span><strong>' + res[1] +  '</strong><br>' + res[2] + '</span></div>');
                    } else {
                        jQuery(this).append('<div class="bx-caption"><span><strong>' + title + '</strong></span></div>'); 
                    }
                }
			});
		}
		...
		el.redrawSlider = function(){
            ...
            // Custom Hack subtract 10px for caption box?!
            jQuery('.bx-caption').outerWidth(getSlideWidth() - 10);
		}
